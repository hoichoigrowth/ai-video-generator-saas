...
class ClaudeScreenplayAgent(BaseAgent):
    """Claude Anthropic powered screenplay formatting agent with enhanced error handling"""
    
    async def process(self, script_text: str, custom_instructions: Optional[str] = None) -> Dict[str, Any]:
        """Process script text into screenplay format using Claude"""
        ...
        try:
            ...
            if not llm:
                raise AgentProcessingError("ClaudeScreenplayAgent", "Claude LLM not configured")
            
            # Sanitize and validate input
            sanitized_script = sanitize_prompt(script_text, max_length=8000)
            if len(sanitized_script) < 100:
                raise AgentProcessingError("ClaudeScreenplayAgent", "Script text too short for processing")
            
            # Prepare prompt
            prompt = CLAUDE_SCREENPLAY_PROMPT.format(script=sanitized_script)
            if custom_instructions:
                prompt += f"\n\nAdditional Instructions: {custom_instructions}"
            
            # Make the API call with retries
            self.logger.info(f"[{processing_id}] Starting Claude screenplay processing")
            ...
            usage_metadata = getattr(result, 'response_metadata', {}).get('usage', {})
            
            response_data = {
                "processing_id": processing_id,
                "content": screenplay_content,
                "provider": "claude",
                "model": getattr(llm, 'model', 'claude-3-opus'),
            ...
            self.logger.info(f"[{processing_id}] Claude screenplay processing completed in {processing_time:.2f}s")
            return response_data
            
        except Exception as e:
            error_msg = f"Claude screenplay formatting failed: {str(e)}"
            self.logger.error(f"[{processing_id}] {error_msg}")
            
            if "rate_limit" in str(e).lower():
                raise ModelAPIError("claude", f"Rate limit exceeded: {str(e)}")
            elif "api_key" in str(e).lower():
                raise ModelAPIError("claude", f"Invalid API key: {str(e)}")
            else:
                raise AgentProcessingError("ClaudeScreenplayAgent", error_msg)
